...
parameters.declare_entry ("tol", "0.0001",
                          Patterns::Double (0, 1),
                          "tolerance");
                          parameters.declare_entry ("initial amplitude",          // displacement size
                            "0.05",                       // default
                            Patterns::Double (0, 1),
                            "fraction of the domain radius used as initial bump");
  
parameters.declare_entry ("viscoelastic relaxation time", // τ  in seconds
                            "1.0",
                            Patterns::Double (1e-6, 1e6),
                            "material relaxation time (eta / E)");
...
amplitude = parameters.get_double ("initial amplitude");
relaxation_time = parameters.get_double ("viscoelastic relaxation time");
...
system_matrix.add(time_step/ relaxation_time,B_matrix);
...
std::vector<std::string> names = {"Ux","Uy","Uz"};
    std::vector<
        DataComponentInterpretation::DataComponentInterpretation>
        comp (dim, DataComponentInterpretation::component_is_part_of_vector);
		
//data_out.add_data_vector(solution, names, DataOut<dim>::type_dof_data, comp);
//for (unsigned int i=0; i<dim; ++i)
  //		data_out.add_data_vector(solution.block(i),
    //                       "D_" + Utilities::int_to_string(i));
...
const double R_domain = 1.0;                         // outer radius
...
{
    const unsigned int  i  = pair.first;
    const Point<dim>   &X  = pair.second;
    const double        r  = X.norm();          // distance from centre

    Tensor<1,dim> disp;

    if (r > 1e-12)                              // regular nodes
    {
        const double radial_factor =
            amplitude * std::sin(numbers::PI * r / R_domain) / r;

        for (unsigned int d = 0; d < dim; ++d)
            disp[d] = radial_factor * X[d];     // radial bump
    }
    else                                         // node at r = 0
        for (unsigned int d = 0; d < dim; ++d)
            disp[d] = 0.0;
    ...
    for (unsigned int d = 0; d < dim; ++d)
        old_solution.block(d)(i) = disp[d];
}

solution = old_solution;
old_sol = old_solution;
old_displacement = displacement;      // keep magnitude array consistent
