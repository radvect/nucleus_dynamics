...
// Получим соответствие между глобальными DoF и координатами
//std::map<types::global_dof_index, Point<dim>> support_points;
//DoFTools::map_dofs_to_support_points(mapping, dof_handler, support_points);

// Инициализация displacement как модуль координаты (например, радиус-вектор)
//for (const auto &dof_point_pair : support_points)
//{
//  const unsigned int dof_index = dof_point_pair.first;
//  const Point<dim> &pt = dof_point_pair.second;

//  displacement[dof_index] = std::sin(10 * pt[0]); // или, например, std::sin(pt[0]) и т.д.
//}
}
...
for (const auto &dof_point_pair : support_points)
{
  ...
  // Только x-компонента
  old_solution.block(0)(dof_index) = 0.01 * std::sin(pt[0]); // пример
  old_solution.block(1)(dof_index) = 0.01 * std::sin(pt[0]);
  old_solution.block(2)(dof_index) = 0.01 * std::sin(pt[0]);
}
solution = old_solution;
old_sol = old_solution; 
