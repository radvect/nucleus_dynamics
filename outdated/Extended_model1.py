...
U = fem.functionspace(domain, ("Lagrange", 1, (domain.topology.dim,)))  # Displacement is vectorial
print(U.ufl_element())
...
# Output XDMF
xdmf = io.XDMFFile(domain.comm, "pressure_map.xdmf", "w")
xdmf_vector = io.XDMFFile(domain.comm, "displacement_map.xdmf", "w")
...
# Compute displacement (u) from pressure gradient
u_ufl = -ufl.as_vector([ufl.grad(p_n)[0], ufl.grad(p_n)[1]])  # Negative gradient for displacement
...
for step in range(num_steps):
    ...
    # Write output to XDMF for pressure and displacement
