...
# Cytoplasm: keep ~0 visualization 
...
ds_n = ufl.Measure("ds", domain=msh_n)  
...
F_rhs = c_coupling * p_n * ufl.div(v) * dx_n  

# traction = 0 on nucleus boundary
...
with io.XDMFFile(MPI.COMM_WORLD, "data/nucleus_submesh_viscoelastic.xdmf", "w") as xdmf_n, \
    ...
    for k in range(num_steps):
        ...
        if MPI.COMM_WORLD.rank == 0:
            print(f"[step {k+1}/{num_steps}] Newton it={n_it}")
