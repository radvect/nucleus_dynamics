...
with io.XDMFFile(MPI.COMM_WORLD, out_path, "w") as xdmf:
    ...
    boundary_tol = 0.08
    boundary_mask = r_all >= (r_all.max() - boundary_tol)
    ###############################################3
    for k in range(num_steps):
        ...
        A_vals = A.x.array.copy()
############################GRAPH
        A_b = A_vals[boundary_mask]
        X_b = coords_all[boundary_mask]

        imax_b = np.argmax(A_b)
        Amax_b = A_b[imax_b]
        x_peak_b = X_b[imax_b].copy()

        times.append(t)
        Amax_boundary_list.append(float(Amax_b))
        peak_boundary_xyz_list.append(x_peak_b)
############################33
        if MPI.COMM_WORLD.rank == 0:
